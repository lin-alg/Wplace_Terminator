...
(function () {
  ...
  GM_addStyle(`
    ...
    .wplace_btn { padding:8px 10px; border-radius:8px; border:none; cursor:pointer; background:rgba(255,255,255,0.04); color:inherit; font-weight:600; white-space:nowrap; }
    ...
    /* Dropdown styles */
    .wplace_dropdown_wrapper { position: relative; }
    .wplace_dropdown_menu {
        position: absolute;
        top: 100%;
        left: 50%;
    ...
    .wplace_dropdown_menu.show { display: flex; flex-direction: column; }
    .wplace_dropdown_item {
        padding: 6px 12px;
        cursor: pointer;
        font-size: 12px;
        color: #ccc;
    ...
    .wplace_dropdown_item:hover { background: rgba(255,255,255,0.1); color: #fff; }
  `);
  ...
  panel.innerHTML = `
    ...
    <div class="row">
      ...
      <div class="wplace_dropdown_wrapper">
        <button id="wplace_theme_btn" class="wplace_btn ghost">${t('theme_default')}</button>
        <div id="wplace_theme_menu" class="wplace_dropdown_menu">
            <div class="wplace_dropdown_item" data-val="default">${t('theme_default')}</div>
            <div class="wplace_dropdown_item" data-val="dark">${t('theme_dark')}</div>
        </div>
      </div>
  ...
  document.body.appendChild(panel);
    ...
    (function initThemeFromStorage() {
        try {
            const stored = localStorage.getItem('theme') || 'custom-winter';
            const apply = () => {
                const btn = panel.querySelector('#wplace_theme_btn');
                if (!btn) return;
                if (stored === 'dark') {
                    btn.textContent = t('theme_dark');
                    panel.classList.add('wplace_theme_dark');
                } else {
                    btn.textContent = t('theme_default');
                    panel.classList.remove('wplace_theme_dark');
                }
            };
            apply();
            setTimeout(apply, 0);
  ...
  // Dropdown Logic
  const themeBtn = panel.querySelector('#wplace_theme_btn');
  const themeMenu = panel.querySelector('#wplace_theme_menu');
  const themeItems = panel.querySelectorAll('.wplace_dropdown_item');
  ...
  if (themeBtn && themeMenu) {
    ...
    document.addEventListener('click', (ev) => {
      if (themeMenu.classList.contains('show') && !themeBtn.contains(ev.target) && !themeMenu.contains(ev.target)) {
        themeMenu.classList.remove('show');
      }
    });
  }
  ...
  (function makePanelDraggable(el) {
    ...
    el.addEventListener('pointerdown', (ev) => {
      if (ev.pointerType === 'mouse' && ev.button !== 0) return;
      if (ev.target && (ev.target.closest('input') || ev.target.closest('button') || ev.target.closest('.resize-handle') || ev.target.closest('.wplace_dropdown_item'))) return;
      ev.preventDefault();
      dragging = true;
      pid = ev.pointerId;
